...
> **ESAM** 解决SSB，做知识迁移
>
> **SimIter** 如何将多模态数据整合到ID base推荐模型中，提出了SimIter压缩高维的多模态数据用于拼接
>
> **RecGPT** LLM提取用户兴趣，另一个LLM细化用户感兴趣的标签，放入召回三塔模型带来提升，LLM还会给出解释，吸引用户点击
>
> **PEPNET** 解决多任务多场景，核心思想是用重要的特征做裁判。其中的PPNET让任务塔对每个用户使用不同的参数，EPNET将场景信息融入到共享表示，值得借鉴。
...
在训练时，输入图像$x$会被编码成一个较短的向量$z$，再被解码回另一幅长得差不多的图像$\hat x$。网络的学习目标是让重建出来的图像和原图像尽可能相似.
...
VQ-VAE通过**codebook码本，其行号称为码字**，将连续的潜在向量离散化，也就是一个embedding table 对编码器的输出向量找出嵌入空间（codebook）中的**最近邻**，随后替换。
...
+ **量化器**：通过查找码本，将z替换为离它最近的码本向量$z_q$，argmin操作不可导的，理论上Decoder的梯度无法穿过Quantization回到Encoder，因此引入了**编码器梯度直通（STE）**，反向传播时假装量化操作不存在$z_q=z_e$
+ **解码器**：尝试将码本向量重建为原始输入$x`$



VQ-VAE的损失训练通常由三部分组成：

1. 第一部分是`重构损失`：$L_{recon} = ||x - \hat x||²$，要求decoder尽可能还原输入

2. 第二部分是`codebook loss`：$||sg[z_e] - e_k||²$，它让 codebook 向量 $e_k$ 靠近 Encoder 输出的 $z_e$，只更新codebook。

3. 第三部分是`承诺损失`：$β ||z_e - sg[e_k]||²$ 它让 Encoder 输出不要到处乱跑，而是主动靠近某个 codebook 向量，只更新encoder。



VQ-VAE一次量化精度不足，不管$z_e$多复杂，最后只用一个$e_k$去近似，如果想提升量化精度，那么直观的想法是**扩大codebook或者增加维度**，使得模型参数激增。

### RQ-VAE

RQ-VAE的提出最初旨在改进自回归编码器图像生成的质量和效率。

RQ-VAE通过引入**残差量化**来解决这个问题，核心思想是**由粗到精的逐层逼近原始表示**，在给定固定码本大小的前提下，递归量化编码器的残差。通过组合多层码字极大扩展了表达能力，同时也无需巨大的码本。
...
编码过程中可以使用 `Sinkhorn` 算法解决冲突，**尽量保持距离相近的同时，让不同的码字使用概率相等**。

对映射到同一个id的sid，首先计算样本与该层所有码本向量的距离，随后将距离转化为初始分配概率，距离越小分配权重越大，随后对矩阵（样本与码字）每一行每一列**反复归一化**，得到**双随机矩阵**，增加码字使用均匀的约束。由于行和列相互归一化会破坏前者的约束，因此需要多次迭代，减小误差。news_rec中只对最后一层使用sk。
...
+ 对大量的物品进行聚类得到K个聚类中心，也就是码本。
+ 对任何一个新的物品向量，找到最接近的聚类中心，并用该中心的索引来表示这个物品向量
+ 计算簇内的距离平均值，作为新的聚类中心
+ 目标是最小化簇内距离平方和
...
为了解决K-Means的码本爆炸问题，可以**PQ**的思路：分而治之。**将一个高维向量切分为多段，为每一段独立训练一个小的K-menas码本，一个物品向量就表示为多个码本索引的组合，可以用很小的存储表示海量的组合。**
...
+ **增加多样性正则**，统计一段时间内码字的使用频率分布，与均匀分布计算KL散度作为惩罚，或最大化离散后验q(z|x)的熵
+ **调参，训练时使用较大的batch size、充分的训练步数**，分阶段训练：先固定解码器只训练编码器和码本，使其学到离散表示，再释放解码器一起优化，以防止解码器过早主导重构而忽略潜向量。
+ **使用EMA算法更新码本**，在VQ-VAE第二版代码中提到能够使码本的更新更加平滑稳定。维护两个指数滑动平均，$$ N_i^{(t)}=\gamma N_i^{(t-1)}+(1-\gamma)n_i^{(t)} $$ $$ m_i^{(t)}=\gamma m_i^{(t-1)}+(1-\gamma)\sum_{j} z_{i,j}^{(t)} $$，前者是一轮mini-batch中某个码本向量的累计使用次数，后者是分配到这个码本向量的encode输出向量（潜向量）之和，最后更新码本向量$$ e_i^{(t)}=\frac{m_i^{(t)}}{N_i^{(t)}} $$
...
+ 辅助序列：由**随时间缓慢变化的类别特征**构成（用户使用的语言.....），为**每个连续不变区间保留最早的一个条目**来压缩这些时间特征，然后将结果**合并到主时间序列**中，由于这些时间序列**变化非常缓慢**，**这种做法不会显著增加整体序列长度**。
...
传统推荐训练 `impression level training`，对用户的历史不断训练，导致复杂度达到$\sum_{i} n_i \left(n_i^2 d + n_i d_{\text{ff}} d\right)$，其中$n_i$是token数， $d$嵌入维度，$d_{ff}$隐藏层维度
...
HSTU采用生成式训练`Generative Training` ，将整条用户行为序列作为一个样本一次性训练，使多个预测目标共享同一次 Transformer 编码，从而把训练复杂度降低到$O(N^2)$，减少了一个量级的计算开销，即
...
因此作者实现了`fully raggified attention`
...
+ **减少注意力模块之外的线性层数量**
+ **大量算子融合**，将多个算子的独立计算合并为一个算子计算，避免频繁读写显存。
...
+ 召回侧new source表示将模型作为一路找回，replace表示完全替代其他的生成器，作为一路召回的效果要优于替代整条链路。
...
工业级大规模推荐模型（Large-scale Recommendation Models，LRMs）需要在严格的效率约束下，同时对用户的**长行为序列**和**异构非序列特征**进行联合建模。然而，现有大多数模型通常采用**解耦式**处理流程：首先使用类似 LONGER 的**、基于查询令牌（query token）的序列压缩器**对长序列进行压缩，随后再通过**类似 RankMixer 的令牌混合模块**，将压缩后的序列表征与稠密特征进行**融合**。这种设计限制了模型的表征能力以及不同特征之间交互的灵活性。



+ 现有架构中的序列在进行序列压缩时，常依赖过于**简化的查询表示**（DIN，DIEN），仅由少量的候选特征或全局特征压缩长行为序列。例如，DIN只是用候选物品作为query，判断历史中哪些是重要的，但是真正决定用户行为的还有许多别的特征（用户年龄，搜索词....）。但如果使用**多个query**，又会导致计算量增加、不满足延迟等问题。
+ 压缩后的序列表示与异构特征的交互发生在**模型后期**，导致二者之间的交互较为浅层且隐式，有些重要信息可能已经丢失了
+ 传统架构由两个相对独立的模块组成：序列压缩器+特征交互器，**增加模型参数时，通常只能单独增强其中一部分**
...
尽管人们普遍认为**多模态数据**具有提升模型准确率的潜力，但许多大规模工业推荐系统，包括淘宝展示广告系统，其模型仍然主要依赖**稀疏的 ID 特征**

主要是因为**多模态带来的收益不足以抵消其部署的成本**，这些成本包括预训练编码器，为新增物品生成表示，对现有服务器和系统进行升级。





为引入多模态，需要**一方面提升其带来的性能收益，另一方面尽可能降低部署成本**，为此需要解决三个问题

+ **预训练任务的设计**：需要设计合适的预训练任务使多模态数据能包含语义信息
+ **多模态表示的整合**：ID特征与多模态表示之间存在一些固有差异，例如训练轮次不同，因此需要有效地将多模态表示整合到ID模型中
+ **系统的设计**：需要及时将表示应用于下游任务中
...
`SCL`，semantic-aware contrastive learning，负责学习多模态表示



![image-20260807164201756](./assets/image-20260807164201756.png)





学习到的表示需要能够区分**语义相似度**，在电商平台中用户的查询与其交互的物品通常高度相似，因此定义训练对为用户查询和交互的物品——`(query text, text of purchased item) (img , img of purchased item)`。负样本从**mini-batch**中随机抽取

为了提升模型的表现，作者认为可以增加负样本的数量，受[MoCo](Momentum Contrast for Unsupervised Visual Representation Learning. In Proceedings of the IEEE/CVF Conference on Computer Vision and  Pattern Recognition. 9726–9735.)的启发，SCL使用两个encoder——**query encoder，key encoder**。前者是经由反向传播训练用于下游任务的encoder，使用前文中定义的pair训练；后者负责生成历史物品的embedding存入**memory bank**，为了保证embedding空间的稳定，key encoder通过动量法更新，$\theta_k \leftarrow m\theta_k + (1 - m)\theta_q$
...
一种将多模态表示与推荐模型整合的最直接方法，是将目标物品以及历史物交互品的多模态表示，与它们对应的ID Embedding进行**拼接**，拼接后的特征会进一步经过用户行为序列建模模块，并最终输入到MLP进行预测。



作者发现：

+ **简化多模态表示的使用方式能够提升模型性能**

+ **基于ID的模型与基于多模态表示的模型在训练epoch上有差异**，前者为了防止过拟合一般只训练一个epoch，而后者则会训练多个epoch。作者搭建了一个仅使用多模态作为输出特征的模型，经过实验发现，使用多模态数据的模型能够从同一数据集上的多轮训练中受益，相比之下，基于 ID 的模型存在“单 epoch 过拟合”现象

  ![image-20260809165108815](./assets/image-20260809165108815.png)



为了简化多模态表示的使用，作者提出了`SimTier`，SimTier首先计算目标物品与历史点击物品多模态表示之间的**相似度**，si = vi · vc，i = 1, ..., L，随后将相似度分数划分为**N个预定义空间**，并**统计**每个区间内相似度分数数量，由此可以得到一个**N维的向量**。由此，SimTier将高维的多模态表示压缩为一个N维的向量，这个向量随后可以与其他向量拼接送入到MLP中。



![image-20260809170409902](./assets/image-20260809170409902.png)



为了解决多模态表示与ID表示训练上的差异，作者提出了`MultimodAl Knowledge Extractor（MAKE）`，将多模态相关参数的优化与 ID 特征的优化解耦。

这里首先经过**Din**模块处理目标物品和历史点击物品的多模态表示，得到一个向量v，随后将这个向量送入到4层的**MLP**中与点击物品做**二分类预测**，经过多轮训练后取v，mlp中间层表示和原来的ID embedding拼接。就是相当于单独训练一个多模态表示。



![image-20260809170427726](./assets/image-20260809170427726.png)
...
阿里开发的LLM推荐系统，实现了`agentic rec`，模型主要贡献在于提升了召回的性能，同时给予了推荐结果的可解释性。



RecGPT中的召回模型使用的是三塔召回，左边的Tag塔是RecGPT独有的，RecGPT通过**理解商品和用户信息**把大模型的能力浓缩进了Tag塔

<img src="./assets/v2-82facee2043a93c61f01824c28087a6c_1440w.jpg" alt="img" style="zoom:67%;" />



Tag代表的是用户感兴趣的的商品标签，它由LLM提取而来，RecGPT首先利用大模型来挖掘用户兴趣，随后把用户兴趣输入到另一个大模型来生成商品标签。

![img](./assets/v2-cdb0dda9eb9b8303521ac5e2e17887ec_1440w.jpg)

## **用户兴趣挖掘模块**



**用户兴趣挖掘模块**会利用LLM的能力把**用户的属性和行为历史抽象**成用户的兴趣点。它的prompt把这个任务描绘的很清晰。

<img src="./assets/v2-708704a1cc495085e89979134b01de99_1440w.jpg" alt="img" style="zoom:67%;" />
...
![img](./assets/v2-039a4355e6f6c927de95c68ddfb8af8f_1440w.jpg)



## 商品标签预估模块

**商品标签预估模块**是要进一步把**用户兴趣细化**，变成**更细粒度**的，更适合推荐系统召回层使用的**商品标签**



<img src="./assets/v2-c02d5daa0df1ebc5725e5ffabbbc3592_1440w.jpg" alt="img" style="zoom:67%;" />
...
对于用户行为序列，RecGPT采取了一些列序列压缩方案

1. **只使用有明确兴趣指向的行为**，比例购买，加入购物车，喜欢等，对于弱兴趣意图比如点击，阅读评论不加入行为序列
2. **商品信息压缩**，只保留关键的能代表商品信息的属性
3. **对用户行为进行时间上和item级别上的压缩**，如固定时间段内的重复点击合并，经常同时出现的item聚合在一起，样例如下所示![img](./assets/v2-dbf286190e5bfd363dd604a4a2bf7b1c_1440w.jpg)

## 推荐可解释性

RecGPT进一步使用LLM给出了推荐原因，把推荐原因展现给用户，有可能进一步提升用户的购买兴趣

<img src="./assets/v2-b13a7b06fad48939c585226ca22732f6_1440w.jpg" alt="img" style="zoom:67%;" />



线上推理时，为了保证时延，对“用户兴趣x商品集合”**预处理**



## 模型微调和评估

对上述的三个模型可以做**fine tuning**，淘宝准备了16.3k个训练样本进行模型微调，模型评估时使用Human-LLM协同评估。三个模型对于数据有不同的**标准**。

RecGPT经过实验发现，经过fine tuning后的模型表现能力更强，即使不进行fine tuning，模型本身的能力也足够，例如开源的**DeepSeek-R1**就是非常好的选择
...
`PEPNET`是一个高效的，即插即用的结构，它将**多场景和多目标理解为用户在不同的情境下和物品交互**，**更准确地进行个性化建模**有助于捕捉用户在不同情境下对物品的偏好程度，从而缓解问题。如果直接把个性化输入作为普通特征，那么经过网络的传播后，其影响会变得微弱，为此`PEPNET`让**个性化信息能控制模型的内部表示和计算过程（底层embedding共享表示和高层DNN隐藏层）**。
