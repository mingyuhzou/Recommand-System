...
2. 第二部分是`codebook loss`：$||sg[z_e] - ze_k||²$，它让 codebook 向量 $z_q$ 靠近 Encoder 输出的 $z_e$，只更新codebook。
...
对映射到同一个id的sid，首先计算样本与所有码字的距离，随后将距离转化为初始分配概率，距离越小分配权重越大，随后对矩阵（样本与码字）每一行每一列**反复归一化**，得到**双随机矩阵**，增加码字使用均匀的约束。由于行和列相互归一化会破坏前者的约束，因此需要多次迭代，减小误差。news_rec中只对最后一层使用sk。
...
+ **使用EMA算法更新码本**，在VQ-VAE第二本代码中提到能够使码本的更新更加平滑稳定。维护两个指数滑动平均，$$ N_i^{(t)}=\gamma N_i^{(t-1)}+(1-\gamma)n_i^{(t)} $$ $$ m_i^{(t)}=\gamma m_i^{(t-1)}+(1-\gamma)\sum_{j} z_{i,j}^{(t)} $$，前者是一轮mini-batch中某个码本向量的累计使用次数，后者是分配到这个码本向量的encode输出向量（潜向量）之和，最后更新码本向量$$ e_i^{(t)}=\frac{m_i^{(t)}}{N_i^{(t)}} $$
