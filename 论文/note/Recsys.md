...
然而，VQ-VAE在处理高保真度数据时面临一个瓶颈：**VQ-VAE想要用一个码字一次性表示特征，但是若要精确表示复杂的输入，就需要增加码本的维度，当物品数量较大时就需要使用多个码字区分，这会带来巨大的计算和存储开销**。
...
对映射到同一个id的sid，首先计算样本与所有码字的距离，随后将距离转化为初始分配概率，距离越小分配权重越大，随后对矩阵（样本与码字）每一行每一列**反复归一化**，增加码字使用均匀的约束。由于行和列相互归一化会破坏前者的约束，因此需要多次迭代，减小误差。
...
梯度归一化算法（`GradNorm`），根据梯度损失计算task样本权重的方法，降低学习速度快的task的样本权重，加快学习速度慢的task的样本权重。不受任务数量限制，仅需少量训练迭代即可完成调优；动态调整梯度范数，使不同任务以相似的训练速度进行学习。
